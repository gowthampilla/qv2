...
export function Sidebar({ admin = false }: { admin?: boolean }) {
  ...
  return (
    <aside className="sticky top-0 z-20 hidden border-[#2A2A2A] bg-[#050505]/88 backdrop-blur-xl md:block md:h-screen md:w-64 md:border-r">
          ...
          {nav.map((item) => {
            ...
            return (
              <a
                ...
                href={item.href}
                className={cn(
                  "group flex min-w-fit items-center gap-2 rounded-xl border border-transparent px-3 py-2 text-sm text-[#8A8A8A] transition-colors",
                  "hover:border-[#2A2A2A] hover:bg-[#111111] hover:text-[#F5F5F5]"
                )}
              >
